...
async function ensureTables() {
  ...
  const tables = [
    `CREATE TABLE IF NOT EXISTS sessions (
      id TEXT PRIMARY KEY, name TEXT, state_json TEXT,
      created_at TEXT DEFAULT (datetime('now')), completed_at TEXT,
      question_count INTEGER DEFAULT 0, guessed_type INTEGER, guessed_wing INTEGER,
      guessed_wing_name TEXT, guessed_summary TEXT, final_scores TEXT,
      actual_type INTEGER, actual_wing INTEGER,
      correct_type INTEGER DEFAULT 0, correct_wing INTEGER DEFAULT 0, notes TEXT
    )`,
    `CREATE TABLE IF NOT EXISTS questions (
      id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT NOT NULL,
      question_number INTEGER NOT NULL, option_a TEXT NOT NULL, option_b TEXT NOT NULL,
      informs TEXT, reasoning TEXT, scores_snapshot TEXT, picked TEXT,
      chosen_text TEXT, unchosen_text TEXT, response_time_ms INTEGER, rating INTEGER,
    ...
    `CREATE TABLE IF NOT EXISTS reports (
      id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT NOT NULL,
      question_number INTEGER NOT NULL, rating INTEGER, complaint TEXT,
      option_a TEXT, option_b TEXT, created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (session_id) REFERENCES sessions(id)
    )`,
    `CREATE TABLE IF NOT EXISTS motion_reports (
      id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT NOT NULL,
      session_id_a TEXT NOT NULL, session_id_b TEXT, report TEXT NOT NULL,
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (session_id_a) REFERENCES sessions(id)
    )`
  ];
...
async function saveMotionReport(type, sessionIdA, sessionIdB, report) {
  await ensureTables();
  await getDb().execute({
    sql: 'INSERT INTO motion_reports (type, session_id_a, session_id_b, report) VALUES (?, ?, ?, ?)',
    args: [type, sessionIdA, sessionIdB || null, report]
  });
}

async function getMotionReport(type, sessionIdA, sessionIdB) {
  await ensureTables();
  const result = await getDb().execute({
    sql: 'SELECT report, created_at FROM motion_reports WHERE type = ? AND session_id_a = ? AND (session_id_b = ? OR (session_id_b IS NULL AND ? IS NULL)) ORDER BY created_at DESC LIMIT 1',
    args: [type, sessionIdA, sessionIdB || null, sessionIdB || null]
  });
  return result.rows[0] || null;
}

async function getMotionReports(sessionId) {
  await ensureTables();
  const result = await getDb().execute({
    sql: 'SELECT * FROM motion_reports WHERE session_id_a = ? OR session_id_b = ? ORDER BY created_at DESC',
